...
local targetCP = 'https://api.linuxlatbot.tk/lookgroup.php?key='.. config.bot_api_key
...
function api.sendKeyboard(chat_id, text, keyboard, parse_mode)
...
function api.resolveUsernameTG(username)

--	local url = BASE_URL .. '/getChat?chat_id=' .. username
	
--	local dat, code = performRequest(url)
	
	local dat, code = performRequest('https://linuxlatbot.tk/cplist/resolveuser.php?userid=' ..URL.escape(username))
	...
	if res.result.last_name and res.result.username then
	   return '🔰📋 *Infomacion del usuario:*\n\n👤 *Nombre:* '..res.result.first_name:escape()..'\n👤 *Apellido:* '..res.result.last_name:escape()..'\n👤 *Usuario 🆔:* '..res.result.id..'\n🔸*Alias:* @'..res.result.username:escape()..''
	elseif not res.result.last_name and res.result.username and not res.result.title then
	   return '🔰📋 *Infomacion del usuario:*\n\n👤 *Usuario:* '..res.result.first_name:escape()..'\n👤 *Usuario 🆔:* '..res.result.id..'\n🔸*Alias:* @'..res.result.username:escape()..''
	...
	elseif res.result.title and res.result.username then
	   ...
	   ..'\nTítulo: ' ..res.result.title:escape()
	   ..'\nAlias: @' ..res.result.username:escape()
	end
end
end

function api.obtener_pruebas(user_id)

	local url = 'https://linuxlatbot.tk/cplist/evidence.php?userid=' ..URL.escape(user_id)
	...
	if not res.ok or not res then
			return 'no existen pruebas de esa id o no es una id de cepero'
	else
	return res
	end
end

function api.res_linuxapiuser(usuario)

	local url = 'https://linuxlatbot.tk/cplist/resolveuser.php?username=' ..URL.escape(usuario)
...
function api.res_linuxapid(usuario)

	local url = 'https://linuxlatbot.tk/cplist/resolveuser.php?userid=' ..URL.escape(usuario)
...
function targetcp(chat_id)

	local url = targetCP .. '&chat_id=' .. URL.escape(chat_id)
...
function api.sendAudio(chat_id, audio, reply_to_message_id, duration, performer, title, caption)
...
function api.sendVideo(chat_id, video, reply_to_message_id, duration, performer, title, caption)
...
function api.sendVoice(chat_id, voice, reply_to_message_id, caption)
