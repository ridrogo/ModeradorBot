...
local function doKeyboard_antispam(chat_id)
    ...
    local humanizations = {
        ...
        ['AntispamPro'] = _('Mensajes de Canales y Grupos publicos')
    }
    for field, value in pairs(config.chat_settings['antispam']) do
        if field == 'links' or field == 'forwards' or field == 'AntispamPro' then
            local icon = '✅'
...
• ❌ = *Not allowed*
            ...
            if blocks[1] == 'toggle' then
                if blocks[2] == 'forwards' or blocks[2] == 'links' or blocks[2] == 'AntispamPro' then
