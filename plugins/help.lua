...
local function do_man()
    local keyboard = {}
    keyboard.inline_keyboard = {
        {
       		{text ='📋 Manual del Bot', callback_data = '!man'},
        },
    	{
    		{text = '👥 Instrucciones para agregarlo a tu grupo', url = 'http://telegram.me/GroupButlerEsp/1'},
        },
        {
    	...
    	{
    		{text = '✅ Grupo de Soporte', url = 'https://telegram.me/joinchat/CsanyT7Z5Wxk8eO_se0BCA'},
    		{text = '✅ GitHub', url = 'https://github.com/ridrogo/ModeradorBot'},
    	},	
    	{	
    		{text = 'Spam', callback_data = '!spamerhelp'},
    		{text = 'Flood', callback_data = '!fhelp'},
    	},
        {
       		{text ='📋 Uso exclusivo del Desarrollador\n(@Webrom)', callback_data = '!com'},
        },
    	{	
    		{text = 'Gban', callback_data = '!gbanhelp'},
    		{text = 'Abuso', callback_data = '!abusohelp'},
    	},
        {
       		{text ='📋 Comandos Exclusivos', callback_data = '!com'},
        },
        {
            {text = '✅ Busquedas con Google', callback_data = '!googlehelp'},
		},
		{
        	{text = '✅ Recordatorios Inline', callback_data = '!recohelp'},
		},
		{
            {text = '✅ Spoilers', callback_data = '!spoilerhelp'},
        	{text = '✅ Spotify', callback_data = '!spotihelp'},
		},
		{
            {text = '✅ Topicos', callback_data = '!topichelp'},
        	{text = '✅ Wikipedia', callback_data = '!wikihelp'},
		},
		{
            {text = '✅ Reglas Inline', callback_data = '!ruleshelp'},
        	{text = '✅ Calculadora Pro', callback_data = '!calchelp'},
		},
		{
            {text = '✅ Traductor', callback_data = '!ruleshelp'},
        	{text = '✅ Sinónimos', callback_data = '!calchelp'},
		},
		{
		    {text = '🔙 Salir del Manual', callback_data = '!salirman'}
      }
	}
	return keyboard
end
...
--    end
 ...
 if blocks[1] == 'spamhelp' then
    api.sendKeyboard(msg.chat.id, 'Presiona el botón para saber sobre el SPAM', {inline_keyboard = {{{text = '📋 Ayuda sobre el SPAM', callback_data = '!infospam'}}}}, true)
 end
 if blocks[1] == 'man' or 'start' then
    api.sendKeyboard(msg.chat.id, 'Presiona un botón para conocer mas sobre el bot, su uso y comandos', do_man(), true)
--    api.sendMessage(msg.chat.id, 'Pronto _Manual completo_ del bot con *todos los comandos disponibles* y su modo de uso.', true)
 end
end
...
function plugin.onCallbackQuery(msg, blocks)
    ...
    local text, keyboard_type, answerCallbackQuery_text
        ...
        if query == 'man' then
            message = 'Manual Completo del bot'
            api.answerCallbackQuery(msg.cb_id, message, true)
            return
        end
        if query == 'spamerhelp' then
            message = 'Hacer SPAM es enviar grupos y/o canales sin consentimiento previo de un admin, el bot te expulsará cuando envies links de telegram.me o telegram.me/joinchat, y/o el alias de un canal o supergrupo'
            api.answerCallbackQuery(msg.cb_id, message, true)
            return
        end
        if query == 'fhelp' then
            message = 'El flood consiste en el envió repetitivo de algún mensaje, pueden ser palabras, imagenes, stickers o videos, el bot podra banear o expulsar al usuario que haga flood.'
            api.answerCallbackQuery(msg.cb_id, message, true)
            return
        end
        if query == 'gbanhelp' then
            message = 'El Gban consiste en el Baneo Global de un usuario, es decir, en todos los grupos donde se encuentre el bot, dicho usuario no podra entrar al grupo, ni ser miembro del mismo.'
            api.answerCallbackQuery(msg.cb_id, message, true)
            return
        end
        if query == 'abusohelp' then
            message = 'Si un usuario abusa en el uso del bot, podrá ser advertido, pero si continua se le bloqueara en el uso del bot, además podrá ser baneado del grupo'
            api.answerCallbackQuery(msg.cb_id, message, true)
            return
        end
        if query == 'com' then
            message = 'Comandos Exclusivos del bot'
            api.answerCallbackQuery(msg.cb_id, message, true)
            return
        end
        if query == 'googlehelp' then
            message = 'Uso: /google tu busqueda.\n Puedes hacer una busqueda de lo que quieras en el buscador Google, se mostrarán los primeros 5 resultados, comando reservado para admins.'
            api.answerCallbackQuery(msg.cb_id, message, true)
            return
        end
        if query == 'man' then
        ...
        if query == 'salirman' then
            user = msg.from.first_name
            message = 'Manual Completo del bot'
            api.answerCallbackQuery(msg.cb_id, message, true)
            api.editMessageText(msg.chat.id, msg.message_id, '*Visto el manual por '..user..'*', 'string')            
            return
        end
        if query == 'infospam' then
            user = msg.from.first_name
            message = 'Hacer SPAM es enviar grupos y/o canales sin consentimiento previo de un admin, el bot te expulsará cuando envies links de telegram.me o telegram.me/joinchat, y/o el alias de un canal o supergrupo'
            api.answerCallbackQuery(msg.cb_id, message, true)
            api.editMessageText(msg.chat.id, msg.message_id, '*Vista la info por '..user..'*', 'string')            
            return
        end
    if query == 'back' then
    	keyboard_type = 'main'
